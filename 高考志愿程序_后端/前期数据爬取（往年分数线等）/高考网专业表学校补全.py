...
def get_text(content):
    ...
    for con in content:
        content_text_son=[]
        re_obj1=re.search(u"[\u4e00-\u9fa5]+、?[\u4e00-\u9fa5]+",con)#专业名称
        content_text_son.append(re_obj1.group())
        con=con[re_obj1.span()[1]:-1]+con[-1]
        re_obj2=re.search(u"[\u4e00-\u9fa5]+[（]?[(]?-?[\u4e00-\u9fa5]+[)]?[）]?",con)#大学名称
    ...
    return content_text
school_names=['中国石油大学','中国地质大学','中国矿业大学','北京师范大学','中国人民大学']
conn = pymysql.connect(host='39.96.168.183', user='root', passwd='123456', db='gaokao', charset='utf8')
cursor=conn.cursor(cursor=pymysql.cursors.DictCursor)
cursor.execute('select 学校名称 from major_supplement')
conn.commit()
school_supp=cursor.fetchall()
conn.close()
for dic in school_supp:
    school_names.append(dic['学校名称'])
flag_province=1
flag_kelei=1
flag_year=2012
flag_page=1
school_names=['中国矿业大学(徐州)','北京师范大学-香港浸会大学联合国际学院']
    ...
    for keleiID in range(flag_kelei,3):
        ...
        for year in range(flag_year,2018):
            ...
            for school_name in school_names:
                while 1:
                    url="http://college.gaokao.com/spepoint/a"+str(provinceID)+"/s"+str(keleiID)+"/y"+str(year)+"/o"+school_name+"/p"+str(page)+"/" 
                    page=page+1
                    while 1: 
                        try:
                    ...
                    try:
                        ...
                        print('正当'+school_name+"时遇到Exception: {}".format(e))
                        continue
                    ...
                    for con_text in content_text:
                        ...
                        if con_text[3]=='--':
                            con_text[3]='Null'
                        try:
                            sql="insert into majors_copy values('"+str(con_text[0])+"','"+str(con_text[1])+"',"+str(con_text[2])+', Null ,'+str(con_text[3])+",'"+str(con_text[4])+"','"+str(con_text[5])+"',"+str(con_text[6])+",'"+str(con_text[7])+"' , Null , Null , Null , Null)"
                            cursor.execute(sql)
                            conn.commit()
                        except Exception as e:
                            #print("在"+con_text[1]+con_text[0]+"遇到Exception: {}".format(e))
                            with open ('err_log.txt','a+',encoding='utf-8') as f:
                                f.writelines("在"+con_text[1]+con_text[0]+"遇到Exception: {}\n".format(e))
                                f.close()
                            continue
